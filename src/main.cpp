...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PurpleCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PurpleCoin Signed Message:\n";
...
{
    // PurpleCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // PurpleCoin: 1 day
static const int64 nTargetSpacing = 2.5 * 60; // PurpleCoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PurpleCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, purplecoin - DarkGravity v2, written by Evan Duffield - evan@purplecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, purplecoin - DarkGravity v3, written by Evan Duffield - evan@purplecoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // PurpleCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PurpleCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PurpleCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PurpleCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PurpleCoinMiner(CWallet *pwallet)
{
    printf("PurpleCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("purplecoin-miner");
    ...
    try { loop {
        ...
        printf("Running PurpleCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PurpleCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PurpleCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - DARKCOIN **********
    Copyright 2014, Darkcoin Developers 
        eduffield - evan@purplecoin.io
        InternetApe - kyle@purplecoin.io
*/
